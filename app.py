...
def chat():
    ...
    # Placeholder bot response (replace with AI response)
...
def generate_response(user_input, faiss_index):
    ...
    context = retrieve_context(user_input, faiss_index, top_k=2)
    prompt_body = f"User: {user_input}\nChatbot:" if not context else f"Previous messages:\n{context}\n\nUser: {user_input}\nChatbot:"
